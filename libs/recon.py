...
# Modified versions of original routines

def recon_alt(cfg, sinogram):
    # If doing the recon, load the projection data, do the recon, and save the resulting image volume.
    # sinogram = load_prep(cfg)

    # cfg.recon.reconType is the recon function's name
    imageVolume3D = feval("gecatsim.reconstruction.pyfiles." + cfg.recon.reconType, cfg, sinogram)

    imageVolume3D = scaleReconData(cfg, imageVolume3D)

    if cfg.recon.saveImageVolume:
        raw = saveImageVolume_alt(imageVolume3D)

    return raw


def saveImageVolume_alt(imageVolume3D):
