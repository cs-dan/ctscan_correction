...
def proj_reverse_2(args1, args2):
    anatomy = args1
    inp_file = args2
    if anatomy.lower() == 'h':
        FOV = 220.16
    ...
    raw_img = inp_file.reshape([512, 512, 1])
...
def proj_setup(args1, args2): #, args3):
    anatomy = args1
    inp_file = args2
    # type = args3
    if anatomy.lower() == 'h':
    ...
    sinogram = DD2FanProj(nrdetcols, x0, y0, xds, yds, xCor, yCor, viewangles, nrviews, sinogram, nrcols, nrrows, originalImgPtr)
    sinogram = sinogram*pixsize
    # rawwrite("C:\\prgs\\ct-proc\\temp\\"+type+"sinogram.raw", sinogram)
    return sinogram
